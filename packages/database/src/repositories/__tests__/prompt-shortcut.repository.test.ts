...
describe('PromptShortcutRepository', () => {
  ...
  it('should correctly save and fetch custom shortcuts', async () => {
    const mockList = [{ id: 'custom-1', icon: '🤖', name: '测试', content: '测试内容' }]
  ...
  it('should overwrite existing array on save', async () => {
    await repo.saveShortcuts([{ id: 'a', icon: 'a', name: 'a', content: 'a' }])
    await repo.saveShortcuts([{ id: 'b', icon: 'b', name: 'b', content: 'b' }])
