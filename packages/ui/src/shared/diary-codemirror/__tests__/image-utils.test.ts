...
import { parseImageMarkdown, buildImageMarkdown, clampWidth } from '../utils/image-utils'
...
describe('parseImageMarkdown ↔ buildImageMarkdown round-trip', () => {
  const cases = [
    { alt: 'alt text', src: 'attachment/a.png', width: 500 },
    { alt: 'alt', src: 'image.png', width: undefined },
    { alt: '', src: 'attachment/wide.png', width: 800 },
    { alt: 'screenshot', src: 'attachment/pasted-2024.png', width: 480 }
  ] as const

  it.each(cases)('round-trips $src with width=$width', ({ alt, src, width }) => {
    const markdown = buildImageMarkdown(alt, src, width)
    const parsed = parseImageMarkdown(markdown, 0)
    expect(parsed).not.toBeNull()
    expect(parsed).toMatchObject({ alt, src, width })
    expect(buildImageMarkdown(parsed!.alt, parsed!.src, parsed!.width)).toBe(markdown)
  })

  it('round-trips width syntax embedded in diary text', () => {
    const diary = 'hello\n\n![after](attachment/after.jpg | 320)\n\nworld'
    const imageLine = '![after](attachment/after.jpg | 320)'
    const lineStart = diary.indexOf(imageLine)
    const parsed = parseImageMarkdown(imageLine, lineStart)
    expect(parsed).toMatchObject({
      alt: 'after',
      src: 'attachment/after.jpg',
      width: 320,
      from: lineStart,
      to: lineStart + imageLine.length
    })
    expect(buildImageMarkdown(parsed!.alt, parsed!.src, parsed!.width)).toBe(imageLine)
  })
})
