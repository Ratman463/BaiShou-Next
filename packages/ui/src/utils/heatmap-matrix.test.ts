...
describe('heatmap-matrix', () => {
  ...
  it('should map counts to the correct dates in the matrix', () => {
    ...
    const jan1st = matrix[1].find(
      (cell) =>
        cell.date.getFullYear() === 2024 && cell.date.getMonth() === 0 && cell.date.getDate() === 1
    )
    expect(jan1st).toBeDefined()
    expect(jan1st?.count).toBe(5)

    // Find Dec 31st in row 2 (Tuesday)
    const dec31st = matrix[2].find(
      (cell) =>
  ...
  it('should filter by month when month parameter is provided', () => {
    ...
    const jan15 = matrix[1].find(
      (cell) =>
        cell.date.getFullYear() === 2024 && cell.date.getMonth() === 0 && cell.date.getDate() === 15
    )
