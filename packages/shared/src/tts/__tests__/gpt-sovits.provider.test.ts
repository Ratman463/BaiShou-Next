...
describe('GptSovitsProvider', () => {
  ...
  describe('synthesize', () => {
    ...
    it('should throw TtsApiError when refAudioPath is missing', async () => {
      ...
      await expect(provider.synthesize(invalidRequest, mockConfig)).rejects.toThrow('GPT-SoVITS 需要指定参考音频路径 (refAudioPath)');
    });
    ...
    it('should throw TtsApiError when API returns error status', async () => {
      ...
      await expect(provider.synthesize(mockRequest, mockConfig)).rejects.toThrow('GPT-SoVITS API 合成失败: Invalid path');
    });
    ...
    it('should throw TtsApiError when connection to service fails', async () => {
      ...
      await expect(provider.synthesize(mockRequest, mockConfig)).rejects.toThrow(TtsApiError);
      await expect(provider.synthesize(mockRequest, mockConfig)).rejects.toThrow('GPT-SoVITS 无法连接到服务: Connection refused');
    });
  });
});
