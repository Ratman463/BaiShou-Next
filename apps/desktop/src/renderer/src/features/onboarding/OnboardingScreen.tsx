...
export const OnboardingScreen: React.FC = () => {
  ...
  const ONBOARDING_PAGES = [
    ...
    },
    {
      ...
      desc: t(
        'onboarding.philosophy_desc',
        '我们相信文字不仅是信息的载体，更是灵魂的切片。通过智能压缩，让跨越时空的对话成为可能。'
      ),
      color: '#7EC8E3'
    },
    {
      ...
      desc: t(
        'onboarding.compression_desc',
        '独创的 AI 压缩算法，将繁杂的日记提炼为纯净的记忆向量。'
      ),
      hasChart: true,
      color: '#64B5F6'
    },
    {
      ...
      desc: t(
        'onboarding.ai_config_desc',
        '白守支持多种 AI 服务提供商。请在设置中配置您的 API Key，以启用 AI 伙伴功能。'
      ),
      isAiConfig: true,
      color: '#BA68C8'
    },
    {
      ...
      desc: t(
        'onboarding.storage_desc',
        '请选择一个存放您灵魂备份的地方。后续所有数据都将只留存在此文件夹。'
      ),
      isStorage: true,
      color: '#FFB74D'
    },
    {
      ...
      desc: t(
        'onboarding.import_desc',
        '如果您有之前的备份数据，可以在完成引导后通过设置中的"数据管理"功能导入。'
      ),
      isImport: true,
      color: '#4DB6AC'
    },
    {
      ...
      desc: t(
        'onboarding.privacy_desc',
        '您的隐私像白纸般纯洁。我们承诺不上传、不归档您的任何私密思想。'
      ),
      isLast: true,
      color: '#81C784'
    }
            ...
            >
              ...
              {page.isAiConfig && (
                <div className={styles.storageBox}>
                  ...
                  <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                    {t(
                      'onboarding.ai_config_steps',
                      '完成引导后，请前往：设置 → AI 服务配置，添加您的 API Key。支持 OpenAI、Claude、Gemini 等多种服务。'
                    )}
                  </div>
                </div>
              )}
              ...
              {page.isStorage && (
                <div className={styles.storageBox}>
                  <div className={styles.pathLabel}>
                    {t('onboarding.current_storage', '当前存储位置')}
                  </div>
              ...
              {page.isImport && (
                <div className={styles.storageBox}>
                  ...
                  <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                    {t(
                      'onboarding.import_steps',
                      '完成引导后，请前往：设置 → 数据管理 → 导入备份，选择您的 ZIP 备份文件即可恢复数据。'
                    )}
                  </div>
                </div>
              )}
          ...
          <div className={styles.btnGroup}>
            ...
            {currentPage.isLast ? (
              <button className={styles.btnPrimary} onClick={handleFinish} disabled={isFinishing}>
                {isFinishing
                  ? t('common.loading', '完成中...')
                  : t('onboarding.get_started', '开始旅程')}
