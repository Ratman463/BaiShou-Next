          ...
          {[...chat.messages].map((msg, index, arr) => {
            ...
            return (
              <ChatBubble
                ...
                onReadAloud={
                  msg.role === 'assistant'
                    ? (content) => handleTtsReadAloud(content, msg.id)
                    : undefined
                }
                ...
                onDelete={async () => {
                  const ok = await dialog.confirm(
                    t(
                      'agent.chat.delete_msg_confirm',
                      '您确定要删除这条消息历史吗？此操作不可逆。'
                    ),
                    ...
                    ? async () => {
                        if (typeof window !== 'undefined' && window.electron) {
                          try {
                            ...
                            const originalTitle =
                              currentSession?.title || currentAssistant?.name || '对话'
                            const title = `${originalTitle} (${t('agent.chat.branch', '分支')})`
                            const newSessionId = await window.electron.ipcRenderer.invoke(
                            ...
                            if (newSessionId) {
                              ...
                              if (loadSessions) {
                                await loadSessions(
                                  true,
                                  currentAssistant?.id ? String(currentAssistant.id) : undefined
                                )
                              }
                              // 导航到新会话
                              const astId = currentAssistant?.id ? String(currentAssistant.id) : ''
                              navigate(
                                `/chat/${newSessionId}${astId ? `?assistantId=${astId}` : ''}`
                              )
                            }
                          } catch (e: any) {
                            toast.showError(
                              e?.message || t('agent.chat.branch_failed', '分支创建失败')
                            )
                          }
                        }
                      }
