...
export function getSummaryManager() {
  ...
  const summaryManager = new SummaryManagerService(summaryRepo, fileSync, summarySync)
...
export function resetCachedManager(): void {
  _cachedManager = null
  // 同时重置 Queue 初始化状态，让 SummaryQueueService 下次调用时重新绑定新的 DB 连接
      ...
      try {
        ...
        > = {
          ...
          en: {
            ...
            diary: '[Diary]',
            subTitle: (months) =>
              `Includes key life events and memories from the past ${months} months`,
            slangs: [
