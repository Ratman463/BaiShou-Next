...
export class LegacyMigrationService {
  ...
  public async migrate(sourceDir: string, targetWorkspaceDir: string): Promise<void> {
    ...
    try {
      ...
      if (stat && stat.isFile()) {
        ...
        logger.info(
          '[LegacyMigration] Restored device_preferences.json via LegacyImportService (full field mapping)'
        )
      }
    } catch (e: any) {
    ...
    if (legacyAvatarsStat && legacyAvatarsStat.isDirectory()) {
      ...
      logger.info(
        `[LegacyMigration] Restored ${files.length} assistant avatars via AttachmentManager`
      )
    }
    ...
    logger.info(
      `[LegacyMigration] Found ${agentDbs.length} agent.sqlite, ${baishouDbs.length} baishou.sqlite`
    )
    ...
    try {
      async function mergeTable(txClient: any, alias: string, tableName: string) {
        ...
        const selectCols = intersectCols
          .map((c: string) => {
            if (
              tableName === 'summaries' &&
              ['start_date', 'end_date', 'generated_at'].includes(c)
            ) {
      ...
      await fsp.unlink(backupDbPath).catch(() => {})
      logger.error(
        '[LegacyMigration] Fatal error during database merging, successfully rolled back whole database!',
        e as any
      )
      throw e
    }
  ...
  public async isLegacyAppRoot(sourceDir: string): Promise<boolean> {
    const agentDbPresent = await fsp
      .stat(path.join(sourceDir, '.baishou', 'agent.sqlite'))
      .catch(() => null)
    const vaultRegPresent = await fsp
      .stat(path.join(sourceDir, '.baishou', 'vault_registry.json'))
      .catch(() => null)
