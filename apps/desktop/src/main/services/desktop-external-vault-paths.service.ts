...
export async function getExternalJournalsDirectoryInfo(options?: {
  ...
  const journalFileCount =
    includeFileCounts && scanDir
...
export async function getExternalSummariesDirectoryInfo(options?: {
  includeFileCounts?: boolean
}): Promise<{
  path: string | null
  defaultPath: string
