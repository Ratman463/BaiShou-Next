...
export const LanTransferScreen: React.FC = () => {
  ...
  const startServer = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        t('common.error', '错误'),
        t('lan_transfer.server_start_failed', '启动服务器失败')
      )
    }
  }, [dbReady, lanSyncService, t])
    ...
    async (device: LanDevice) => {
      ...
      try {
        ...
        Alert.alert(
          ...
          success
            ? t('lan_transfer.send_success', '发送成功')
            : t('lan_transfer.send_failed', '发送失败')
        )
      } catch (e) {
        ...
        <View style={[styles.container, { backgroundColor: colors.bgApp }]}>
          ...
          >
            ...
            <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>
              {t('lan_transfer.title', '局域网传输')}
            </Text>
            <TouchableOpacity onPress={startScan} disabled={scanning}>
              <Text
                style={[
                  styles.scanButton,
                  { color: scanning ? colors.textSecondary : colors.primary }
                ]}
              >
                {scanning
                  ? t('lan_transfer.scanning', '扫描中...')
                  : t('lan_transfer.scan_devices', '扫描设备')}
              </Text>
            </TouchableOpacity>
          </View>
          ...
          <ScrollView style={styles.content} indicatorStyle={scrollIndicatorStyle(isDark)}>
              ...
              <View style={styles.serverStatus}>
                <View style={styles.statusRow}>
                  ...
                  <Text style={[styles.statusText, { color: colors.textPrimary }]}>
                    {serverRunning
                      ? t('lan_transfer.running', '运行中')
                      : t('lan_transfer.stopped', '已停止')}
                  </Text>
                </View>

                <View style={styles.portRow}>
                  <Text style={[styles.portLabel, { color: colors.textPrimary }]}>
                    {t('lan_transfer.port', '端口')}:
                  </Text>
                  <TextInput
                    style={[
                      styles.portInput,
            ...
            <View style={[styles.section, { backgroundColor: colors.bgSurface }]}>
              <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                {t('lan_transfer.discovered_devices', '发现的设备')}
              </Text>
              ...
              {devices.length === 0 ? (
                <View style={styles.emptyContainer}>
                  ...
                  <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
                    {t('lan_transfer.no_devices', '暂未发现设备')}
                  </Text>
                  <Text style={[styles.emptySubText, { color: colors.textSecondary }]}>
                    {scanning
                      ? t('lan_transfer.scanning_hint', '正在扫描局域网设备...')
                      : t('lan_transfer.scan_hint', '点击右上角扫描按钮搜索设备')}
                  </Text>
                </View>
              ) : (
                devices.map((device) => (
                  <View
                    key={device.id}
                    style={[styles.deviceItem, { backgroundColor: colors.bgSurfaceHighest }]}
                  >
            ...
            <View style={[styles.section, { backgroundColor: colors.bgSurface }]}>
              <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                {t('lan_transfer.transfer_history', '传输记录')}
              </Text>
              <View style={styles.emptyContainer}>
                <Text style={styles.emptyIcon}>📋</Text>
                <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
                  {t('lan_transfer.no_history', '暂无传输记录')}
                </Text>
              </View>
            </View>
          </ScrollView>
